...
{
    // Search for bundled gama-local in order of preference:
    // 1. Same directory as executable
    // 2. ../external/bin (relative to executable)
    // 3. System PATH (default "gama-local")
    
    QString appDir = QCoreApplication::applicationDirPath();
    QStringList searchPaths = {
        appDir + "/gama-local",
        appDir + "/../external/bin/gama-local",
        appDir + "/../bin/gama-local",
        "/usr/local/bin/gama-local",
        "/usr/bin/gama-local"
    };
